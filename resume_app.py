...
# def process_resume_matching(resume_path, requirement_path):
#     data = ResumeParser(resume_path).get_extracted_data()
#     have_skills = {skill.lower().translate(str.maketrans('', '', string.punctuation)) for skill in data['skills']}
    
#     req_skills = resumeparse.read_file(requirement_path)
#     skills = {skill.lower() for skill in req_skills['skills']}
    
#     matched_skills = have_skills & skills
#     per = (len(matched_skills) / len(skills)) * 100
...
#     if resume_input and requirement_input:
#         resume_path = io.BytesIO(resume_input.read())
#         requirement_path = io.BytesIO(requirement_input.read())
...
def process_resume_matching(resume_path, requirement_path):
    ...
    matched_skills = have_skills.intersection(skills)
    per = (len(matched_skills) / len(skills)) * 100
